...
static unsigned char TextParser_reallocateString(struct Logger* logger, char** string, size_t* oldLength, size_t step) {
    if (!logger || !string || !oldLength || !step)
        return 1;
    char* newString = NULL;
    size_t i;
...
static unsigned char TextParser_splitExpression(struct Logger* logger, const char* const string, size_t* startIndex,
    ...
    if (!logger || !string || !startIndex || !leftOperandString || !leftCounter 
...
static unsigned char TextParser_addItemToRightOperand(struct Logger* logger, struct Pair* pair,
    ...
    if (!logger || !pair || !itemString || !itemStringLength)
        return 1;
    size_t i = 0;
...
static unsigned char TextParser_parseItemsArrayString(struct Logger* logger, struct Pair* pair, char* itemsString,
    ...
    if (!logger || !pair || !itemsString || !itemsStringLength)
        return 6;
    unsigned char state = 0;
...
static unsigned char TextParser_addPair(struct Logger* logger, struct TextParser* textParser, char* leftOperandString,
    ...
    if (!logger || !textParser || !leftOperandString || !leftCounter || !rightOperandString || !rightCounter)
        return 1;
    size_t j;
...
static unsigned char TextParser_parseTextResource(struct Logger* logger, struct TextParser* textParser,
    ...
    if (!logger || !textParser || !textResource)
        return 1;
